...
//! in what it does once the supervisor answers. It never mounts the TUI, and it
//! never becomes the supervisor: `phoxal-supervisor` is a separate executable and the
//! systemd unit starts it directly, so readiness and the watchdog belong to
//! the supervisor, not to this command.
