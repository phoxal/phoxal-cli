...
//! compatibility contract (#936). Before the strict typed parser runs, the CLI
//! probes the declared revision and answers the one question the parser cannot:
//! *which side is out of date*.
//!
//! There are exactly two ways a tag can be wrong in a version sense, and they
//! have opposite fixes:
//!
//! - a **newer revision** of a family this CLI knows (`phoxal/robot/v1`): the
//!   document was authored for a later train, so the tool moves;
//! - a **superseded spelling** this CLI recognizes but no longer accepts
//!   (`robot/v0`, the unnamespaced 0.53-era grammar tag): the document predates
//!   this train, so the document moves.
//!
//! Both are rejections - a superseded spelling is named, never accepted - and
//! both name the file, the declared tag, and the exact edit. A wrong-*family*
...
use phoxal_runtime_contract::RobotSchema;
...
impl DocumentKind {
    ...
    pub const fn supported(self) -> &'static [&'static str] {
        const ROBOT: &[&str] = &[RobotSchema::V0.as_str()];
        match self {
            Self::Robot => ROBOT,
        }
    }

    /// Spellings this family used on an earlier train and no longer accepts.
    ///
    /// This is not a compatibility shim: nothing here parses. It exists so a
    /// project written against an older CLI is told *what to change* instead of
    /// being handed serde's unknown-variant text, which reads like an authoring
    /// mistake when it is really a stale document.
    #[must_use]
    pub const fn superseded(self) -> &'static [&'static str] {
        match self {
            // The 0.53-era unnamespaced grammar tags, renamed when the document
            // identities became namespaced enums.
            Self::Robot => &["robot/v0"],
        }
    }

    /// The tag a caller should be told to write.
    fn current_tag(self) -> &'static str {
        self.supported()
            .last()
            .copied()
            .expect("every document family supports at least one tag")
    }
}
...
pub fn ensure_supported_revision_str(text: &str, path: &Path, kind: DocumentKind) -> Result<()> {
    ...
    if kind.superseded().contains(&declared) {
        bail!(
            "{path}: document declares schema `{declared}`, which phoxal renamed to \
             `{current}`.\n\nThe document predates this phoxal CLI. Change its `schema:` tag to \
             `{current}` and rebuild.",
            path = path.display(),
            current = kind.current_tag(),
        );
    }
...
mod tests {
    ...
    fn the_family_is_the_supported_tag_without_its_revision() {
        ...
        assert_eq!(DocumentKind::Robot.supported(), &[RobotSchema::V0.as_str()]);
    }
    ...
    /// The inverse direction: a project authored against the 0.53-era CLI. The
    /// tag is not newer, it is older, and telling that operator to update the
    /// CLI would send them the wrong way.
    #[test]
    fn a_superseded_spelling_asks_for_the_document_to_be_retagged() {
        let text = "schema: robot/v0\nrobot: {}\n";
        let error =
            ensure_supported_revision_str(text, Path::new("/proj/robot.yaml"), DocumentKind::Robot)
                .expect_err("the unnamespaced spelling is not accepted");
        let message = format!("{error:#}");
        assert!(message.contains("/proj/robot.yaml"), "{message}");
        ...
        // It must not send the operator to update the tool: the tool is fine.
        assert!(!message.contains("Update the phoxal CLI"), "{message}");
    }
    ...
    fn wrong_family_tag_defers_to_the_strict_parser() {
        ...
        // Both the current spelling and the superseded one are wrong-family
        // here, and neither is a version gap.
