...
impl Theme {
    ...
    /// this CLI (identity, progress, log lines) writes to.
    ///
    /// `--plain`/`--quiet` (via [`crate::progress::current_mode`]) also force
    ...
    /// [`OutputMode::Json`](crate::output_mode::OutputMode::Json) forces it
    /// too, belt-and-suspenders with [`crate::ui::Ui`]'s own json gate.
    #[must_use]
    pub fn detect_stderr() -> Self {
        if !crate::progress::current_mode().allows_progress_drawing() {
