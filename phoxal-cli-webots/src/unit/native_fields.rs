            ...
            if let Some(projection) = cfg.projection {
                fields.assignments.push(NativeWebotsFieldAssignment {
                    ...
                    value: NativeValue::String(
                        match projection {
                            phoxal_utils_simulation::capability::CameraProjection::Planar => {
                                "planar"
                            }
                            phoxal_utils_simulation::capability::CameraProjection::Cylindrical => {
                                "cylindrical"
                            }
                            phoxal_utils_simulation::capability::CameraProjection::Spherical => {
                                "spherical"
                            }
                        }
                        .to_string(),
...
pub fn native_webots_motor_fields(
    cfg: &phoxal_utils_simulation::capability::Motor,
...
mod tests {
    ...
    #[test]
    fn gnss_native_fields_use_webots_gps_names() {
        let sim = SimulationCapability::Gnss(phoxal_utils_simulation::capability::Gnss {
            sampling_period_hz: 10.0,
            accuracy: Some(1.234),
            noise_correlation: Some(0.25),
            resolution: Some(0.01),
            speed_resolution: None,
            speed_noise: None,
        });
        let fields = native_webots_fields_for_capability(&sim);
        ...
        assert!(names.contains(&"noiseCorrelation".to_string()));
        assert!(!names.contains(&"noise".to_string()));
    }

    #[test]
    fn depth_reconciled_with_renderer() {
        // Webots RangeFinder depth supports these native fields directly.
        let sim = SimulationCapability::Depth(phoxal_utils_simulation::capability::Depth {
            resolution: Some(0.01),
            noise: Some(1.0),
            motion_blur: Some(0.5),
            ..Default::default()
        });
        let fields = native_webots_fields_for_capability(&sim);
        let names: Vec<String> = fields
        ...
        assert!(names.contains(&"noise".to_string()));
        assert!(names.contains(&"motionBlur".to_string()));
    }

    #[test]
    fn motor_invalid_pid_length_returns_error() {
        let cfg = phoxal_utils_simulation::capability::Motor {
            control_pid: Some(vec![1.0, 2.0, 3.0, 4.0]),
            ..Default::default()
        };
        let result = native_webots_motor_fields(&cfg);
